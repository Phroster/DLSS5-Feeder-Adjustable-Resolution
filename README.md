...
builds a complete DLSS DLAA "contract" out of what ReShade already has â€” the frame being processed,
the depth buffer, and iMMERSE **LaunchPad**'s optical-flow motion vectors â€” runs a genuine DLSS
...
```
game frame â†’ ReShade effects â†’ [MartysMods_Launchpad] â†’ [DLSS5_Feed] â†’ DLSS5-Feeder:
                                 motion vectors            depth + MV      DLSS DLAA + DLSS 5 NR on a private D3D12 device
                                                                           â†“
                                            neural output written back over the frame â†’ later effects â†’ present
```

### Status

Proven working in **Metro 2033 Redux** (64-bit, D3D11, no native DLSS): the DLSS 5 neural-rendering
add-on reports `feature 18 created â€¦ inline feature 18 evaluation succeeded` at native 4K/1440p,
driven entirely by ReShade depth + LaunchPad motion vectors. This is a first, rough version â€” expect
...
1. **ReShade with add-on support** â€” get it from **https://reshade.me** , run the installer, pick your
...
5. **Turn it on in-game** â€” press **Home** for the ReShade overlay, enable **MartysMods_Launchpad**,
...
That's it. Check `dlss5-feed.log` (next to the game `.exe`) for `feature ready â€¦ DLAA` and
...
> have no DLSS. The bridge â€” **https://github.com/NIGos/dlss5-dx11-bridge/releases** â€” is only for
...
* `dlss5-feed.addon64` registers with the ReShade add-on API. After the `DLSS5_Feed` technique
  ...
  its neural pass â€” it cannot tell the contract is synthetic.
* NGX calls are wrapped in SEH: if the (closed-source) DLSS 5 add-on faults â€” e.g. across a
  resolution change â€” the feed disables itself and the game keeps running, rather than crashing.
...
| iMMERSE **LaunchPad** (`MartysMods_LAUNCHPAD.fx` + `MartysMods/*.fxh` + `Textures/iMMERSE_bluenoise_opt.png`) | from https://github.com/martymcmodding/iMMERSE â€” install it yourself; it is proprietary and is **not** bundled here. |
...
| `mode` | 2 | 0 inert Â· 1 transport test (frame out â†’ back, no NGX) Â· 2 full DLSS path. |
...
(`feature ready: WxH DLAA, flags=â€¦`), `frame N delivered`, a timing line every 600 frames, and a
...
* **DLAA only** â€” render resolution = output resolution = the game's backbuffer. No upscaling perf
  gain yet; a jittered render-at-lower / output-at-higher upscaling mode is future work.
* Estimated motion vectors â†’ temporal artifacts in fast motion; the UI is processed with the scene
...
* **D3D11â†”D3D12 shared-texture / fence transport** adapted from NIGos'
  [dlss5-dx11-bridge](https://github.com/NIGos/dlss5-dx11-bridge) (MIT) â€” not re-hosted here.
...
MIT â€” see [LICENSE](LICENSE). This covers only the code in this repository (`src/`, `shaders/`); the
